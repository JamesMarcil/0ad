...
- You have access to specialist sub-agents via the Agent tool, including (when present in this project) `pm` (requirements/clarifying questions), `perf-guru` (performance impact review), and `qa` (regression risk and test coverage) — check what's available and use them rather than reinventing their function.
