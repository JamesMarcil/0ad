...
public:
	...
	void SendObjectMsg(bool preview)
	{
		...
		if (preview)
			POST_MESSAGE(ObjectPreview, (m_ObjectID.ToStdWstring(), GetScenarioEditor().GetObjectSettings().GetSettings(), m_ObjPos, useTarget, m_Target, g_DefaultAngle, m_ActorSeed, true));
		else
		{
			POST_COMMAND(CreateObject, (m_ObjectID.ToStdWstring(), GetScenarioEditor().GetObjectSettings().GetSettings(), m_ObjPos, useTarget, m_Target, g_DefaultAngle, m_ActorSeed));
			RandomizeActorSeed();
		}
	}
