...
- Replay-based verification (`pyrogenesis_dbg.exe -replay=<commands.txt>`, full per-turn hash check) surfaced a **separate, pre-existing crash** — an `entt::sparse_set` assertion (`"Slot not available"`, `sparse_set.hpp:332`) — that reproduces identically on the `447b4c541f` baseline itself (verified by temporarily swapping in that revision's `ComponentManager.cpp` and rebuilding). This is **not** caused by Phase 2 or by this revert; it's an independent, already-existing defect somewhere in the EnTT entity/component lifecycle that predates today's work.
  - **Bisection so far**: the same replay run against the very first `entt::registry` integration commit (`426728042b`, before any message-dispatch rewrite existed) ran well past the point where it crashes on later commits, with no assertion — evidence the bug was introduced somewhere in the range `(426728042b, 447b4c541f]`, i.e. by `a97cada593` ("Modernize BroadcastMessage and PostMessage using EnTT views") or by `447b4c541f` itself, not by the original registry integration. Not narrowed further than that range. Tracked as a separate open issue — does not block this revert, since it predates and is independent of the Phase 2 work being undone here.
