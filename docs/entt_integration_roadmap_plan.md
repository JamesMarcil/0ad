...
- Replay-based verification (`pyrogenesis_dbg.exe -replay=<commands.txt>`, full per-turn hash check) surfaced a **separate, pre-existing crash** — an `entt::sparse_set` assertion (`"Slot not available"`, `sparse_set.hpp:332`) — that reproduces identically on the `447b4c541f` baseline itself (verified by temporarily swapping in that revision's `ComponentManager.cpp` and rebuilding). This is **not** caused by Phase 2 or by this revert; it's an independent, already-existing defect somewhere in the EnTT entity/component lifecycle that predates today's work.
  - **Bisected**: replaying the same recorded session against each commit between the known-good `426728042b` (original registry integration, no crash) and the known-bad `447b4c541f` isolates the introducing commit to **`a97cada593`** ("Modernize BroadcastMessage and PostMessage using EnTT views") — the only other commit in that range touching `ComponentManager.cpp`. That commit added an **unguarded** `m_Registry.storage<IComponent*>(cid).emplace(enttEntity, component)` call in `ConstructComponent` (no `contains()`/`erase()` check beforehand) — if a component of a given `cid` is ever (re)constructed for an entity ID that already has a live entry in that `cid`'s storage, `entt::sparse_set::try_emplace` hits `ENTT_ASSERT(elem == null, "Slot not available")` (`sparse_set.hpp:332`).
    ...
    - Tracked as a separate open issue, independent of and not blocking the Phase 2 revert above.
    - **Fixed (2026-08-24, commit `a5f3f53d2f`)**: added `CComponentManager::GetComponentStorageId(cid, isLocal)`, which namespaces each per-`cid` `entt::storage<IComponent*>` by locality using a reserved high bit of the storage key, so normal and local entities' components live in physically separate sparse sets regardless of their raw index collision — no change to the `entity_id_t → entt::entity` cast itself was needed. Routed all three call sites that key storage by `ComponentTypeId` (`ConstructComponent`, `FlushDestroyedComponents`, `PostMessage`) through it. Added a regression test (`test_component_manager_local_normal_index_collision` in `test_EnTTFeatureFlag.h`) that allocates a normal and a local entity via the same public counters real gameplay uses (landing on IDs known to collide) and gives both the same component type — confirmed this reproduces the crash reliably without the fix (verified by temporarily stashing it out) and passes with it.
      - Verified: Win32 Debug and Release, both with and without `CONFIG_ENABLE_ENTT_ECS`, full cxxtest suite 490/490. Replaying the session that originally crashed completes cleanly (~22s) against the Release build with the fix — no crash, no hash mismatch. (A Debug-build replay of the same session ran anomalously long — 16+ minutes without completing or crashing — before being abandoned in favor of the much faster Release confirmation; this wasn't investigated further and may just be Debug container-checking overhead compounding with the doubled storage-bucket count, but is called out here rather than silently assumed benign.)
